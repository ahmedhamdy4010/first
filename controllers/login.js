...
router.post('/', (req, res) => {
    ...
    validator.validate(data, (errors, fields) => {
        if (!errors) {
            userModel.validateUser0(req.body.email, req.body.password, function (result) {
                if (!result) {
                    ...
                    console.log(result);
                        if (result.is_admin == 1) {
                            ...
                            req.session.sub_id = result.sub_id;

                            res.redirect('/admin/home');
                        } else {
                            req.session.emp = result.user_id;
                            req.session.sub_id = result.sub_id;
                            console.log('session-is-emp');
